  ...
  $(document).ready(function(){
    $('#table_id').dataTable({
     ...
     "mRender" : function (data) {
      return "<img src='../../images/"+data.post_image+"' class='product-img product-img-p'/>";}
    },
    { mData: 'tr_title'},
    { "mData": null , "width": "5%", "className":"status text-center",
     "mRender" : function (data) {
      if (data.post_featured == 1) {
        return '<span class="badge badge-pill bg-success"><?php echo _YESTEXT; ?></span>';
      }else{
        return '<span class="badge badge-pill bg-warning"><?php echo _NOTEXT; ?></span>';
      }
      }
    },
    { "mData": null , "width": "5%", "className":"status text-center",
     "mRender" : function (data) {
      if (data.post_visibility == 1) {
    ...
    'searchable': false,
    "mRender" : function (data) {
      return "<a data-toggle='modal' data-target='#view-modal' id='showEditModal' class='btn btn-small btn-primary' style='color:#fff' data-id='"+data.post_id+"'>"+EDITITEM+"</a> <a class='btn btn-small btn-danger btn-delete deleteItem' data-url='../controller/delete_post.php?id="+data.post_id+"'>"+DELETEITEM+"</a>";}
    }
    ]
  });
  });
...
$(document).ready(function(){
  $(document).on('click', '#showEditModal', function(e){
      ...
      $('#modal-loader').show();
      $.ajax({
          url: '../controller/get_posts_languages.php',
          type: 'POST',
          data: 'id='+uid,
          dataType: 'html'
      })
